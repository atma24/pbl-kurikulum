            ...
            <div className="flex-1 flex flex-col overflow-hidden relative">
                <header className="h-20 shrink-0 bg-white/80 backdrop-blur-md border-b border-gray-200 flex items-center justify-between px-8 z-10 shadow-sm">
                    ...
                    <div className="flex items-center gap-4 shrink-0">
                        ...
                        {/* UBAH DIV MENJADI LINK INERTIA */}
                        <Link 
                            href={route('profile.edit')}
                            className="block h-8 w-8 rounded-full bg-gray-300 overflow-hidden ml-2 border-2 border-polman-primary hover:opacity-80 transition-opacity focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-polman-primary"
                        >
                            <img src={`https://ui-avatars.com/api/?name=${user?.name || 'User'}&background=008B8B&color=fff`} alt="Profile" className="w-full h-full object-cover" />
                        </Link>
                    </div>
                </header>

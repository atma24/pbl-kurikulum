...
export default function MatrixPage({ cpls, ieas, ppms, cplToPpmMatrix }: Props) {
    // State untuk mengontrol Tab mana yang aktif
    const [activeTab, setActiveTab] = useState<'iea-cpl' | 'iea-ppm' | 'cpl-ppm'>('iea-cpl');

    // --- Handlers (Auto-Save saat checkbox diklik) ---
    const handleSyncCpl = (cplId: number, ieaId: number, isSelected: boolean) => {
        router.post('/matrix/sync-cpl-iea', {
            cpl_id: cplId,
            iea_id: ieaId,
            is_selected: isSelected
        }, { preserveScroll: true }); // Mencegah layar loncat saat dicentang
    };
    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="mb-6">
                ...
                <p className="text-gray-500 text-sm mt-1">Pemetaan relasional otomatis antara IEA, CPL, dan Profil Lulusan (PPM).</p>
            </div>
                    ...
                    <table className="min-w-full border-collapse font-body">
                        <thead>
                            <tr className="bg-polman-neutral">
                                ...
                                {ieas.map(iea => (
                                    <th key={iea.id} className="p-4 border-b text-center text-[10px] font-bold text-polman-primary uppercase min-w-[80px]">{iea.kode}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                                        ...
                                        return (
                                            <td key={iea.id} className="p-4 border-b text-center">
                                                <input 
                                                    type="checkbox" 
                                                    checked={isChecked}
                                                    onChange={(e) => handleSyncCpl(cpl.id, iea.id, e.target.checked)}
                                                    className="w-5 h-5 rounded border-gray-300 text-polman-primary focus:ring-polman-primary cursor-pointer"
                                                />
                                            </td>
                                        );
                                    })}
                                        ...
                                        return (
                                            <td key={iea.id} className="p-4 border-b text-center">
                                                <input 
                                                    type="checkbox" 
                                                    checked={isChecked}
                                                    onChange={(e) => handleSyncPpm(ppm.id, iea.id, e.target.checked)}
                                                    className="w-5 h-5 rounded border-gray-300 text-polman-primary focus:ring-polman-primary cursor-pointer"
                                                />
                                            </td>
                                        );
            ...
            {/* --- TAB 3: RESULTAN CPL x PPM (Read-Only) --- */}
            {activeTab === 'cpl-ppm' && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-x-auto">
                    <div className="p-4 bg-blue-50 border-b border-blue-100 text-sm text-blue-800 flex items-center gap-2 font-body">
                        ...
                        Tabel ini dihasilkan secara otomatis. Jika CPL dan PPM beririsan pada IEA yang sama, relasi akan tercentang.
                    </div>
                    <table className="min-w-full border-collapse font-body">
                        <thead>
                            <tr className="bg-polman-secondary text-white">
                                ...
                                {ppms.map(ppm => (
                                    <th key={ppm.id} className="p-4 border-b border-polman-primary text-center text-[10px] font-bold uppercase min-w-[80px]">{ppm.kode}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {!cpls || cpls.length === 0 ? (
                                ...
                                <tr key={cpl.id} className="hover:bg-gray-50">
                                    ...
                                    {ppms.map(ppm => {
                                        // Baca hasil irisan dari cplToPpmMatrix kiriman backend
                                        ...
                                        return (
                                            <td key={ppm.id} className="p-4 border-b text-center bg-gray-50/30">
                                                {isLinked ? (
                                                    <div className="w-6 h-6 mx-auto bg-polman-secondary rounded flex items-center justify-center text-white font-bold text-xs shadow-sm">✓</div>
                                                ) : (

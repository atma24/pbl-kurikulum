...
export default function PpmPage({ ppms }: Props) {
    ...
    const openEditModal = (ppm: Ppm) => {
        setModalMode('edit');
        setSelectedId(ppm.id);
        setData('deskripsi', ppm.deskripsi);
        clearErrors();
        setIsModalOpen(true);
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (modalMode === 'add') {
            post('/ppm', { onSuccess: () => closeModal() });
        } else {
            patch(`/ppm/${selectedId}`, { onSuccess: () => closeModal() });
        }
    };

    const handleDelete = (id: number, kode: string) => {
        if (confirm(`Apakah Anda yakin ingin menghapus ${kode}?`)) {
            router.delete(`/ppm/${id}`);
        }
    };

    return (
        <AuthenticatedLayout>
            <Head title="Master Data PPM" />

            <div className="flex justify-between items-center mb-6">
                <div>
                    <h2 className="font-headline font-bold text-2xl text-gray-900">Profesi & Peluang Magang (PPM)</h2>
                    <p className="text-gray-500 text-sm font-body mt-1">Kelola data profil profesional lulusan.</p>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden font-body">
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                        {!ppms || ppms.length === 0 ? (
                            <tr><td colSpan={3} className="px-6 py-8 text-center text-gray-500">Belum ada data PPM.</td></tr>
                        ) : (
                            ppms.map((ppm) => (
                                <tr key={ppm.id} className="hover:bg-gray-50">
                                    <td className="px-6 py-4 text-sm font-bold text-polman-primary">{ppm.kode}</td>
                                    <td className="px-6 py-4 text-sm text-gray-700">{ppm.deskripsi}</td>
                                    <td className="px-6 py-4 text-right text-sm font-medium">
                                        <button onClick={() => openEditModal(ppm)} className="text-blue-600 hover:text-blue-900 mr-4">Edit</button>
                                        <button onClick={() => handleDelete(ppm.id, ppm.kode)} className="text-red-600 hover:text-red-900">Hapus</button>
                                    </td>
                                </tr>
                            ))
                        )}
                ...
                <div className="fixed inset-0 flex items-center justify-center p-4">
                    <Dialog.Panel className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl font-body">
                        <Dialog.Title className="font-headline text-xl font-bold text-gray-900 mb-4">
                            {modalMode === 'add' ? 'Tambah Data PPM' : 'Edit Data PPM'}
                        </Dialog.Title>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">Deskripsi PPM</label>
                                <textarea
